...
impl PacketSender {
    ...
    pub fn start(target: Ipv4Addr, ch_rx: Receiver<Message>, pch_tx: Sender<PacketSentMessage>) -> io::Result<JoinHandle<io::Result<()>>> {
        let (transport_sender, _) = transport_channel(1024, Layer4(Ipv4(IpNextHeaderProtocols::Icmp)))?;
        let mut packet_sender = PacketSender { transport_sender, target, pch_tx, ch_rx };
